...
describe "Emay" do
  ...
  describe "#to" do
    let(:cdkey) { 'YOUR_CDKEY' }
    let(:password) { 'YOUR_PASSWORD' }
    let(:url) { "http://sdkhttp.eucp.b2m.cn/sdkproxy/sendsms.action" }
    let(:content) { '测试内容' }
    subject { ChinaSMS::Service::Emay.to phone, content, cdkey: cdkey, password: password }
    ...
    describe 'single phone' do
      ...
      before do
        stub_request(:post, url).
          with(body: {cdkey: cdkey, password: password, phone: phone, message: content}).
